-- Only reconcile-delete rows for entity types present in the push payload.
-- Prevents habits-only pushes from wiping cloud transactions (e.g. SQL import).
...
begin
  ...
  if jsonb_array_length(coalesce(p_payload->'objectives', '[]'::jsonb)) > 0 then
    delete from public.budget_objectives o
    where o.user_id = p_user_id
      and not exists (
        select 1 from jsonb_array_elements(p_payload->'objectives') elem
        where (elem->>'id')::uuid = o.id
      );
  end if;

  if jsonb_array_length(coalesce(p_payload->'category_budget_limits', '[]'::jsonb)) > 0 then
    delete from public.category_budget_limits l
    where l.user_id = p_user_id
      and not exists (
        select 1 from jsonb_array_elements(p_payload->'category_budget_limits') elem
        where (elem->>'id')::uuid = l.id
      );
  end if;

  if jsonb_array_length(coalesce(p_payload->'transactions', '[]'::jsonb)) > 0 then
    delete from public.transactions t
    where t.user_id = p_user_id
      and not exists (
        select 1 from jsonb_array_elements(p_payload->'transactions') elem
        where (elem->>'id')::uuid = t.id
      );
  end if;

  if jsonb_array_length(coalesce(p_payload->'habit_completions', '[]'::jsonb)) > 0 then
    delete from public.habit_completions c
    where c.user_id = p_user_id
      and not exists (
        select 1 from jsonb_array_elements(p_payload->'habit_completions') elem
        where (elem->>'id')::uuid = c.id
      );
  end if;

  if jsonb_array_length(coalesce(p_payload->'habits', '[]'::jsonb)) > 0 then
    delete from public.habits h
    where h.user_id = p_user_id
      and not exists (
        select 1 from jsonb_array_elements(p_payload->'habits') elem
        where (elem->>'id')::uuid = h.id
      );
  end if;

  if jsonb_array_length(coalesce(p_payload->'prayer_logs', '[]'::jsonb)) > 0 then
    delete from public.prayer_logs p
    where p.user_id = p_user_id
      and not exists (
        select 1 from jsonb_array_elements(p_payload->'prayer_logs') elem
        where (elem->>'id')::uuid = p.id
      );
  end if;

  if jsonb_array_length(coalesce(p_payload->'movies', '[]'::jsonb)) > 0 then
    delete from public.movies m
    where m.user_id = p_user_id
      and not exists (
        select 1 from jsonb_array_elements(p_payload->'movies') elem
