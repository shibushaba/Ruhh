...
class PrayerRepository {
  ...
  /// Next salāh to highlight: earliest upcoming unlogged, else latest overdue today.
  Future<PrayerName?> nextUnloggedPrayerByTime({DateTime? now}) async {
    final clock = now ?? DateTime.now();
    final log = await dailyLogFor(todayKey);
    final times = await displayTimesForDay(clock);
    return nextUnloggedPrayer(log: log, times: times, now: clock);
  }
